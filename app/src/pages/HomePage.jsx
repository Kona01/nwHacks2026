...
const HomePage = () => {
  ...
  const [hasMore, setHasMore] = useState(true); // Track if there are more clubs to load
  const gridRef = useRef(null);

  const fetchClubs = (pageNum) => {
    setLoading(true);
    // Use the new paginated endpoint
    fetch(`http://localhost:5000/api/clubs?page=${pageNum}`)
      .then((response) => response.json())
      .then((data) => {
        if (data.length < 50) {
          setHasMore(false); // If we get back fewer than 50, we've reached the end
        }
        
        // Append new data to existing list if page > 1, otherwise set fresh
  ...
  // ... handleLuckyClick logic remains the same ...
            ...
            <div key={club.id} className="club-card-wrapper transition-all duration-300">
              <Link to={`/clubs/${club.id}`} className="group flex flex-col h-full bg-white rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 club-card-shadow">
                ...
                <div className="aspect-[4/3] w-full flex items-center justify-center p-6 relative bg-gradient-to-br from-[#f5f5f5] to-[#e0e0e0]">
                  <img src={club.logo_url || 'https://ires.ubc.ca/files/2019/10/ubc-logo.png'} alt={club.name} className="max-w-[80%] max-h-[80%] object-contain z-10" />
                </div>
